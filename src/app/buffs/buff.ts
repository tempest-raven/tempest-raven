...
export class Buff {
  static attributeMap = new Map<number, [string, (_: number) => number | string | null]>();
  static langStrings: languageObject;
  public attributes = new Map<number, string | number>();
  ...
  localizedDescription(language: sonnyLanguage = "ENGLISH") {
    ...
    const replacerFunc = function (this: Buff, _: string, index: string | number, percent: string) {
    ...
    description = description
      .replaceAll(/\(?100 \* _root\.hackMove2\[(\d+)\]( ?\+ "%")?\)?/g, replacerFunc)
      .replaceAll(/\(?_root\.hackMove2\[(\d+)\] \* 100\)?()/g, replacerFunc);
  ...
  localizedName(language: sonnyLanguage = "ENGLISH") {
    const match = /BUFF_NAME\[(\d+)\]/.exec(this.name);
    if (match !== null) {
  ...
  setAttribute(attributeIndex: number, value: string) {
    ...
    if (attributeIndex !== 25 && !value.startsWith("_root") && !value.startsWith('"')) {
