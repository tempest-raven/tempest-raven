...
export class Item {
  ...
  elementArray = ["Physical", "Magic", "Ice", "Fire", "Lightning", "Earth", "Shadow", "Poison"];
  static slotWeights = [0, 0, 0.14, 0.14, 0.1, 0.14, 0.1, 0.2, 0.2, 0.4];

  public appearance = "NINJA";
  ...
  //Based on the getStat function found in Sonny 2
  private static getStat(s_ratio: number, s_level: number, s_mode: boolean) {
    ...
    if (s_level <= 5) {
      ...
      s_a =
        (-0.0000213 * Math.pow(s_level, 5) +
          0.002 * Math.pow(s_level, 4) -
          0.0693 * Math.pow(s_level, 3) +
          1.43 * Math.pow(s_level, 2) -
          8.8533 * s_level +
          39.5) *
        (s_ratio / 10);
    }
  ...
  get vitality() {
    return Math.ceil(
      (Item.getStat(40, this.level + 3, false) * this.weight() * this.rawStats.HealthUP) / 100,
    );
  }
  get strength() {
    return Math.ceil(
      (Item.getStat(40, this.level + 3, false) * this.weight() * this.rawStats.StrengthUP) / 100,
    );
  }
  get magic() {
    return Math.ceil(
      (Item.getStat(40, this.level + 3, false) * this.weight() * this.rawStats.MagicUP) / 100,
    );
  }
  get speed() {
    return Math.ceil(
      (Item.getStat(40, this.level + 3, false) * this.weight() * this.rawStats.SpeedUP) / 100,
    );
  }
  get focus() {
  ...
  get piercing() {
    return this.rawStats.PIU.map(per =>
      Math.round((100 + 15 * (this.level + 3)) * this.weight() * (per / 70)),
    );
  }
  get defense() {
    return this.rawStats.DIU.map(def =>
      Math.round((100 + 15 * (this.level + 3)) * this.weight() * (def / 70)),
    );
  }
  ...
  buyPrice() {
    if (this.equipSlot < 2) {
      ...
      return Math.round(
        this.valueMultiplier * (this.weight() * 2.5) * this.respecValue(this.level) * 3,
      );
    }
  }
